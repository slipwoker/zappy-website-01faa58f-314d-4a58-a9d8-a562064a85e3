document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        const validators = {
            ...
            text: (value, fieldName) => {
                if (!value.trim()) return `${fieldName || 'This field'} is required`;
                if (value.trim().length < 2) return `${fieldName || 'This field'} must be at least 2 characters`;
                return null;
            },
            textarea: (value, fieldName) => {
                if (!value.trim()) return `${fieldName || 'This field'} is required`;
                if (value.trim().length < 10) return `${fieldName || 'This field'} must be at least 10 characters`;
                return null;
            }
        };
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            const requiredInputs = contactForm.querySelectorAll('[required], input[type="email"], input[type="tel"]');
            
            requiredInputs.forEach(input => {
                clearError(input);
                
                const type = input.type || input.tagName.toLowerCase();
                const name = input.getAttribute('name') || input.getAttribute('placeholder') || 'This field';
                ...
                if (type === 'email') {
                    ...
                    error = validators.textarea(value, name);
                } else if (input.hasAttribute('required')) {
                    error = validators.text(value, name);
                }
    ...
    const animatedElements = document.querySelectorAll('.fade-in, .fade-in-up, .fade-in-left, .fade-in-right, [data-animate]');
    ...
    if (animatedElements.length > 0) {
        ...
        animatedElements.forEach(element => {
            ...
            if (element.classList.contains('fade-in-up') || element.getAttribute('data-animate') === 'up') {
                element.style.transform = 'translateY(30px)';
            } else if (element.classList.contains('fade-in-left') || element.getAttribute('data-animate') === 'left') {
                element.style.transform = 'translateX(-30px)';
            } else if (element.classList.contains('fade-in-right') || element.getAttribute('data-animate') === 'right') {
                element.style.transform = 'translateX(30px)';
            }
        ...
        style.textContent = `
            .fade-in.visible, .fade-in-up.visible, .fade-in-left.visible, .fade-in-right.visible, [data-animate].visible {
                opacity: 1 !important;
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '01faa58f-314d-4a58-a9d8-a562064a85e3',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '01faa58f-314d-4a58-a9d8-a562064a85e3';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '01faa58f-314d-4a58-a9d8-a562064a85e3';
